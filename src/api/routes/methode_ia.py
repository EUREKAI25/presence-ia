"""
Routes Méthode Présence IA — génération du livrable 500€.
"""
import json
import logging
import os
...
@router.get("/admin/methode-ia", response_class=HTMLResponse)
def admin_methode_ia_page(request: Request):
    ...
    for j in recent:
        ...
        if status == "done" and j.get("result", {}).get("deliverable_path"):
            ...
            dl_link = (
                f'<a href="/api/methode-ia/result/{job_id}" target="_blank" '
            ...
            dl_link = '<span style="color:#d69e2e">En cours…</span>'
...
<head>
<meta charset="UTF-8">
<title>Méthode Présence IA — Admin</title>
<style>
  * {{ box-sizing:border-box; margin:0; padding:0 }}
  ...
  .hdr a {{ color:rgba(255,255,255,.75);text-decoration:none;font-size:14px }}
  ...
  input {{ width:100%;padding:9px 12px;border:1px solid #e2e8f0;border-radius:6px;
           font-size:14px;outline:none }}
  input:focus {{ border-color:#667eea }}
  .btn {{ background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;border:none;
          padding:12px 28px;border-radius:8px;font-size:15px;font-weight:700;cursor:pointer }}
  table {{ width:100%;border-collapse:collapse;background:#fff;border-radius:10px;overflow:hidden }}
  th {{ background:#edf2f7;padding:10px 14px;text-align:left;font-size:13px;
...
<div class="hdr">
  ...
  <strong>Méthode Présence IA — Générer un livrable</strong>
</div>
<div class="wrap">

<div style="background:#fff;border-radius:12px;padding:28px;box-shadow:0 1px 4px rgba(0,0,0,.08);margin-bottom:32px">
  <h2 style="font-size:18px;font-weight:800;margin-bottom:20px">Générer un livrable client</h2>
  <form id="form" style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
    ...
    <div style="grid-column:1/-1;display:flex;gap:16px;align-items:center;margin-top:8px">
      <button class="btn" type="button" onclick="launch()">🚀 Générer le livrable</button>
...
<div style="background:#fff;border-radius:12px;padding:24px;box-shadow:0 1px 4px rgba(0,0,0,.08)">
  ...
  <div id="table-wrap">
    <table>
      <thead>
        <tr>
          <th>Entreprise</th><th>Ville</th><th>Métier</th>
          <th>Statut</th><th>Date</th><th>Résultat</th>
        </tr>
      </thead>
...
async function launch() {{
  ...
  document.getElementById('status').textContent = '⏳ Lancement en cours…';

  const r = await fetch('/api/methode-ia/run?token={token}', {{
    method:'POST',
  ...
  document.getElementById('status').textContent = '⏳ Pipeline IA en cours (peut prendre 2-3 min)…';

  const poll = setInterval(async () => {{
    const rp = await fetch('/api/methode-ia/status/' + jobId + '?token={token}');
    const dp = await rp.json();
    if (dp.status === 'done') {{
      clearInterval(poll);
      document.getElementById('status').innerHTML =
        '✅ Terminé ! Score : <strong>' + dp.score + '/10</strong> — ' +
        '<a href="/api/methode-ia/result/' + jobId + '?token={token}" target="_blank">📄 Voir le livrable</a>';
      location.reload();
...
# ── API — lancement pipeline ─────────────────────────────────────────────────

@router.post("/api/methode-ia/run")
...
def _run_job(job_id: str, company_name: str, city: str, business_type: str, website: str):
    try:
        ...
        from src.methode_ia.pipeline import run_pipeline
    ...
    try:
        ...
        if result["ok"]:
            ...
            _JOBS[job_id].update({"status": "error", "error": result.get("error", "Erreur inconnue")})
    except Exception as e:
        log.error("[methode_ia] job %s échoué : %s", job_id, e)
        _JOBS[job_id].update({"status": "error", "error": str(e)})


# ── API — statut + résultat ───────────────────────────────────────────────────

@router.get("/api/methode-ia/status/{job_id}")
...
@router.get("/api/methode-ia/result/{job_id}", response_class=HTMLResponse)
def job_result_html(job_id: str, request: Request):
    ...
    html_content = job["result"].get("deliverable_html", "<p>HTML non disponible</p>")
    return HTMLResponse(html_content)


@router.get("/api/methode-ia/result/{job_id}/json")
...
# ── API — test depuis un prospect V3 existant ─────────────────────────────────

@router.post("/api/methode-ia/from-prospect/{token}")
async def run_from_prospect(token: str, request: Request, background_tasks: BackgroundTasks):
    """Lance le pipeline depuis les ia_results d'un prospect V3 existant."""
    if not _admin_ok(request):
        raise HTTPException(403)
    ...
    try:
        ...
        job_id = str(uuid.uuid4())
        company_name  = p.name or token
        city          = p.city or ""
        business_type = p.profession or ""
        website       = getattr(p, "website", "") or getattr(p, "url", "") or ""
        ...
        background_tasks.add_task(
            _run_job_with_existing,
            job_id, company_name, city, business_type, website, ia_results
        )
        return {"job_id": job_id, "status": "running", "prospect": company_name}
    finally:
        db.close()


def _run_job_with_existing(
    job_id: str,
    company_name: str,
    city: str,
    business_type: str,
    ...
    try:
        ...
        if result["ok"]:
            _JOBS[job_id].update({"status": "done", "result": result, "score": result["score"]})
        else:
